...
classdef COMB < handle
    ...
    methods(Access = public)
        function obj = COMB(sample_delay, RT60, fs)
               ...
               obj.ddl = zeros(obj.sdel, 1); % Set Delay Line
               obj.g = 10 ^ ((-3 * length(obj.ddl)) / (obj.RT60*obj.fs)); % Calculate G Value
        end
        ...
        function out = read(obj)
            % Return current value of comb filter
            out = obj.ddl(obj.idx);
        end
        ...
        function update_RT60(obj, RT60)
            % recalculate g value with new RT60
            obj.g = 10 ^ ((-3 * length(obj.ddl)) / (RT60*obj.fs));
        end

        function update(obj, RT60, fs)
            obj.fs = fs; % Redefine sample rate
            obj.g = 10 ^ ((-3 * length(obj.ddl)) / (RT60*obj.fs)); % Redefine G
        end
